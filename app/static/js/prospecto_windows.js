...
(function () {
    ...
    /* ════════ TAREAS — multi-tarea en ventanas ════════
       El modal de tarea (#crmTaskDetailModal) ya es windowable
    ...
    (function () {
        ...
        var obs = new MutationObserver(function () {
            if (!m) return;
            if (m.classList.contains('ww-windowed') &&
                document.body.style.overflow === 'hidden') {
                document.body.style.overflow = '';
            }
        });
        function watch() {
            var el = document.getElementById('crmTaskDetailModal');
            if (!el || el === m) return;
            m = el;
            obs.observe(el, { attributes: true, attributeFilter: ['class'] });
        }
        if (typeof window.crmReady === 'function') window.crmReady(watch);

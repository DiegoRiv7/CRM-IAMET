...
def _system_prompt(user, config: AsistenteConfig) -> dict:
    ...
    base = (
        ...
        '6j. **Oportunidades abiertas / pendientes de cierre**: "qué '
        'oportunidades están abiertas", "qué falta facturar", "opp '
        'vendidas pero no cerradas", "reporte de cierre", "qué hay '
        'pendiente de cerrar" → usa `oportunidades_abiertas_vendido`. '
        'Renderiza el resultado AGRUPADO por pipeline y por etapa (la '
        'tool ya viene agrupada). Para cada pipeline usa '
        '`### Pipeline: Runrate` / `### Pipeline: Proyecto`. Dentro, '
        '**una sub-sección por etapa** con `#### [Etapa] · N opps · '
        '$Monto`. Luego una tabla markdown así:\n'
        '   ```\n'
        '   | Oportunidad | Cliente | Vendedor | OC del Drive | Próximo paso |\n'
        '   |-------------|---------|----------|--------------|--------------|\n'
        '   | [Título — Cliente — $Monto](opp:ID) | Cliente | Vendedor | OCC-XXX.pdf | Tarea: "Solicitar PO" (vence 28 May) |\n'
        '   ```\n'
        '   **NUNCA inventes nombres de archivos OCC**: usa exactamente lo '
        'que viene en `archivos_occ` (cada string es un nombre real); si '
        'son varios, lístalos separados por coma. Si la lista está vacía, '
        'pon `Sin OC` literal. Para el próximo paso, prefijo "Tarea:" o '
        '"Actividad:" según `tipo`, e incluye la fecha; si `vencida=true` '
        'agrégale " ⚠️ vencida". Si no hay próximo paso, pon `—`. Al '
        'inicio de la respuesta, un bloque `::: kpis :::` con totales '
        'globales: `Total abiertas | N | $Monto MXN` y por pipeline.\n'
